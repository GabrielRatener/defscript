    ...
    if (typeof node === 'object' && node !== null && node.hasOwnProperty('type')) {
      ...
      if (transformers.hasOwnProperty(node.type)) {
        ...
        const context = {
          ...
          // obj?.method?()
          transformSafeChain(node) {
            
            const obInject =
              this.template(`((<%= ref %> = <%= e %>) != null)`, 'expression');
            const fnInject =
              this.template(`(typeof (<%= ref %> = <%= e %>) === 'function')`, 'expression');
            const reverseStack = new Stack();

            const makeId = (name) => new this.node.Identifier({name});

            const transform = (item) => {
              
              let nd = this.transform(item);

              while (reverseStack.length > 0) {
                const top = reverseStack.pop();

                if (top.safe) {
                  const rfVar = this.newVar('rf');
                  const ref = makeId(rfVar);
                  const placeholder = makeId(rfVar);
                
                  const unsafe =
                    (top.type === 'CallExpression') ?
                      new this.node.CallExpression({
                        callee: placeholder,
                        arguments: this.transformAll(top.arguments)
                      }) :
                    (top.type === 'MemberExpression') ?
                      new this.node.MemberExpression({
                        object: placeholder,
                        property: this.transform(top.property),
                        computed: top.computed
                      }) :
                      null

                  return new this.node.ConditionalExpression({
                    test: 
                      (top.type === 'CallExpression') ?
                        fnInject({ref, e: this.transform(top.callee)}) :
                      (top.type === 'MemberExpression') ?
                        obInject({ref, e: this.transform(top.object)}) :
                        null
                    ,
                    consequent: transform(unsafe),
                    alternate: makeId('undefined')
                  })
                }

                switch (top.type) {
                  case 'CallExpression':
                    nd = new this.node.CallExpression({
                      callee: nd,
                      arguments: this.transformAll(top.arguments)
                    });
                    break;
                  case 'MemberExpression':
                    nd = new this.node.MemberExpression({
                      object: nd,
                      property: this.transform(top.property),
                      computed: top.computed
                    })
                    break;
                  default:
                    throw new Error(`Non-chainable node-type "${chainableNode.type}"`);
                }
              }
            ...
            loop:
            while (true) {
              switch (reverseStack.top.type) {
                case "CallExpression":
                  reverseStack.push(reverseStack.top.callee);
                  break;
                case "MemberExpression":
                  reverseStack.push(reverseStack.top.object);
                  break;
                default:
                  break loop;
              }
            ...
            return transform(reverseStack.pop());
          },
...
export const transformers = {
  ...
  Literal(node) {
    ...
    if (node.value === undefined) {
      ...
      return new this.node.Identifier({name: 'undefined'});
    } else {
    ...
    switch (node.operator) {
      case '?':
        return new this.node.BinaryExpression({
          left: this.transform(node.argument),
          right: new this.node.Literal({value: null}),
          operator: '!='
        })
      default:
        return this.pass();
  ...
  PipeExpression(node) {
    if (node.left) {
      return new this.node.CallExpression({
        callee: this.transform(node.consumer),
        arguments: [
          this.transform(node.input)
        ]
      });
    } else {
      const ref = this.newVar('ref');

      return new this.node.SequenceExpression({
        expressions: [
          new this.node.AssignmentExpression({
            left: new this.node.Identifier({name: ref}),
            right: this.transform(node.input),
            operator: '='
          }),
          new this.node.CallExpression({
            callee: this.transform(node.consumer),
            arguments: [
              new this.node.Identifier({name: ref})
            ]
          })
        ]
      });
  ...
  MemberExpression(node) {
    return this.transformSafeChain(node);
  },
  ...
  CallExpression(node) {
    ...
    const placeholders =
      node.arguments
        .map((arg, i) => arg.type === 'Placeholder' ? i : null)
        .filter((arg) => arg !== null);

    const vars = placeholders.reduce((map, offset) => {
      const arg = this.newVar('arg', false);
    ...
    if (placeholders.length > 0) {
      if (node.safe) {
        throw Error("Safe call-expression may not contain placeholders!");
      } else {
        return new this.node.ArrowFunctionExpression({
          id: null,
          async: false,
          generator: false,
          expression: true,
          params: placeholders.map((offset) => {
            const arg = vars.get(offset);

            return new this.node.Identifier({name: arg})
          }),
          body: new this.node.CallExpression({
            callee: this.transform(node.callee),
            arguments: node.arguments.map((argument, offset) => {
              if (argument.type === 'Placeholder') {
                return new this.node.Identifier({name: vars.get(offset)});
              } else {
                return this.transform(argument);
              }
            })
          })
        });
