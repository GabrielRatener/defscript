...
export const transformers = {
    ...
    Function(node) {
        ...
        if (bound) {
            ...
            const context = this.context('method');
            const name =
              (implicitBinding && context && context.data.definition === node) ?
    ...
    ObjectExpression(node) {
        ...
        return injector({
            obj: new this.node.ObjectExpression({
                properties: node.properties.map((p) => this.transform(p))
            })
        });
    },
    ...
    Method(node) {
        ...
        if (node.definition.bound === null) {
            // by default method functions are immutably bound to their
            // host object

            const context = this.context('parent-object');

            this.push({
                state: 'method',
                data: {
                    definition: node.definition,
                    subject: context.data.name
                }
            });    
        }
